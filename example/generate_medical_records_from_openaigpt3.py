...
def generate_text_for_disease(disease):
    ...
    response1 = openai.Completion.create(engine=INSERT_ENGINE, prompt=prompt1, temperature=.3, max_tokens=64)

    prompt2 = prompt1 + response1['choices'][0]['text'] + '. Some special cases also present'
    response2 = openai.Completion.create(engine=INSERT_ENGINE, prompt=prompt2, temperature=.3, max_tokens=128)

    prompt3 = prompt2 + response2['choices'][0]['text'] + '. The main symptomatic differences of ' + disease + ' from similar diseases is that'
    response3 = openai.Completion.create(engine=INSERT_ENGINE, prompt=prompt3, temperature=.3, max_tokens=64)
